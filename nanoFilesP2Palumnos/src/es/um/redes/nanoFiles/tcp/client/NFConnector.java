...
public class NFConnector {
	...
	public boolean downloadFile(String targetFileHashSubstr, File file) throws IOException {
		...
		String fileNameInput = inputMessage.getFileName();
		...
		if(opcodeInput == PeerMessageOps.OPCODE_DOWNLOAD_FROM_OK) {
			System.out.println("File recieved. Checking integrity...");
			File f = new File("aux");
			try {
			    if (!f.exists()) {
			        f.createNewFile();
			    ...
			    e.printStackTrace(); // Manejo de la excepción: imprime la traza de la excepción, pero podrías manejarla de otra forma según tus necesidades.
			}
		...
		} else if (opcodeInput == PeerMessageOps.OPCODE_AMBIGUOUS_DOWNLOAD) {
			System.out.println("Ambiguous download request");
			String[] hashes = fileHashInput.split(":");
			String[] fileNames = fileNameInput.split(":");
			System.out.println("Possible files you are looking for: ");
			for (int i = 0; i < hashes.length; i++) {
				System.out.println("Hash: " + hashes[i] + " Name: " + fileNames[i]);
			}
