...
public class NFControllerLogicDir {
	...
	protected boolean getAndPrintUserList() throws IOException {
		...
		try {
			...
			if(lista==null) {
				...
				System.out.println(String.format("%-20s %-20s", "Username:", "Type:"));
				...
				for(String usr: lista) {
					...
					System.out.println(String.format("%-20s %-20s", nombre, tipo));
				}
	...
	protected boolean getAndPrintFileList() throws IOException {
		...
		if(ficheros!=null){
			result = true;
			System.out.println("Published files:");

			FileInfo.printToSysout(ficheros);
			/*
			System.out.println(String.format("%-30s %-20s %-40s", "Nombre:", "Tamaño:", "Hash:"));
			for(FileInfo fichero : ficheros){
				if (fichero != null) {
					String formattedOutput = String.format("%-30s %-20s %-40s", fichero.fileName, fichero.fileSize + " bytes", fichero.fileHash);
	 ...
	 * 
	 */
	public boolean publishLocalFiles() throws IOException {
		/*
		 * TODO: Comunicarse con el directorio (a través del directoryConnector) para
	 ...
	 */
	public InetSocketAddress getServerAddress(String serverNicknameOrSocketAddr) throws UnknownHostException,IOException {
