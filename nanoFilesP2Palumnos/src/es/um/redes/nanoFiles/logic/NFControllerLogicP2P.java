...
public class NFControllerLogicP2P {
	...
	protected void foregroundServeFiles(NFControllerLogicDir controllerDir) {
		 ...
		 */
		try {
			NFServerSimple serverSimple = new NFServerSimple(controllerDir);
			serverCreated = true;
			serverSimple.run();
			serverCreated = false;
		} catch (IOException e) {
	 ...
	 * 
	 */
	protected boolean backgroundServeFiles(NFControllerLogicDir controllerDir) {
		/*
		...
		if(serverCreated) {
			...
			try {
				server = new NFServer(controllerDir);
				Thread serverThread = new Thread(server);
				serverThread.start();
				serverCreated = true;
				System.out.println("* Background server started");
				return true;
			} catch (IOException e) {
				System.err.println("* ERROR: IOException has occurred");
			}
