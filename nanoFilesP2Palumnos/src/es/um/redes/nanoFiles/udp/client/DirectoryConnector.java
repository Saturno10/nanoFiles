...
public class DirectoryConnector {
	...
	public boolean logIntoDirectory(String nickname) throws IOException {
		...
		String	loginString = message.toString();
		// TODO: 3.Crear un datagrama con los bytes en que se codifica la cadena
		//String loginString = DirMessageOps.OPERATION_LOGIN+"&"+nickname;
		byte[] loginBytes = loginString.getBytes();
		...
		DirMessage messageResponse = DirMessage.fromString(respuestaString);
		if (messageResponse.getOperation().equals(NFDirectoryServer.LOGIN_OK)){
			System.out.println("Login exitoso. SessionKey: " + messageResponse.getNickname());
			sessionKey = Integer.parseInt(messageResponse.getNickname()); 
			success = true;
		}else {
			
			System.err.println("Login fallido. Usuario ya logeado (único error que ahora mismo puede pasar)");
		}
		/*if(respuestaString.contains("&")) {
			...
			System.err.println("No puedes hacer login, si ya estás loggeado.");
		}*/
	...
	public String[] getUserList() throws IOException {
		...
		String userlistString = message.toString();
		byte[] userlistBytes = userlistString.getBytes();
		byte[] respuesta = sendAndReceiveDatagrams(userlistBytes);
		String respuestaString = new String(respuesta);
		DirMessage messageResponse = DirMessage.fromString(respuestaString);
		if (messageResponse.getOperation().equals(NFDirectoryServer.USERLIST_OK)) {
			System.out.println("Userlist exitoso.");
			userlist = messageResponse.getNickname().split(",");
			
		}else {
			System.out.println("Userlist fallido.");
		}
	...
	public boolean logoutFromDirectory() throws IOException {
		...
		message.setNickname(((Integer) sessionKey).toString());
		//System.out.println(message.toString());
		// TODO: 2.Convertir el objeto DirMessage a enviar a un string (método toString)
		String	logoutString = message.toString();
		// TODO: 3.Crear un datagrama con los bytes en que se codifica la cadena
		//String loginString = DirMessageOps.OPERATION_LOGIN+"&"+nickname;
		byte[] logoutBytes = logoutString.getBytes();
		// TODO: 4.Enviar datagrama y recibir una respuesta (sendAndReceiveDatagrams).
		...
		String respuestaString = new String(respuesta);
		DirMessage messageResponse = DirMessage.fromString(respuestaString);
		if (messageResponse.getOperation().equals(NFDirectoryServer.LOGOUT_OK)){
