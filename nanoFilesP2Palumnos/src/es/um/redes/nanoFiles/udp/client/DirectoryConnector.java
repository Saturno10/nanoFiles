...
public class DirectoryConnector {
	//Mensaje login y mensaje de respuesta OK al comando login
	...
	//Temporizador de 2s para que recv no se bloquee para siempre
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) throws IOException {
		...
		int messageAttemps = 0;
		while(messageAttemps < 5) {
			try {
				...
				if (messageAttemps < 5) {
					System.out.println("Timeout excedeed, retrying...");
				}else {
