		...
		case DirMessageOps.OPERATION_SEARCH: {
			...
			case DirMessageOps.OPERATION_FILELIST: {
				...
				if(sessionKeys.containsKey(sessionKey)){
					...
					// HashMap para almacenar los nombres de usuario de los servidores que comparten cada archivo
					HashMap<String, List<String>> fileToUsernames = new HashMap<>();
					...
					for(Map.Entry<Integer,HashMap<String,String[]>> server : publishedFiles.entrySet()) {
						for(Map.Entry<String,String[]> fichero : server.getValue().entrySet()){
							// Agregar el nombre de usuario a la lista correspondiente en el HashMap
							String username = sessionKeys.get(server.getKey());
							fileToUsernames.computeIfAbsent(fichero.getKey(), k -> new ArrayList<>()).add(username);
						}
					}

					// Construir la cadena files
					for (String hash : fileToUsernames.keySet()) {
						String fileName = publishedFiles.get(sessionKey).get(hash)[0];
						String fileSize = publishedFiles.get(sessionKey).get(hash)[1];
						files.append(hash).append(",").append(fileName).append(",").append(fileSize).append(",");
						for (String username : fileToUsernames.get(hash)) {
							files.append(username).append("/");
						}
						// Eliminar la última barra y agregar dos puntos para separar los archivos
						files.setLength(files.length() - 1); // remove last "/"
						files.append(":");
					}
