...
public class NFDirectoryServer {
	...
	public static final String USERLIST_OK = "userlistok";
	public static final String USERLIST_ERR = "userlist_failed";
	...
	private DirMessage buildResponseFromRequest(DirMessage msg, InetSocketAddress clientAddr) {
		...
		case DirMessageOps.OPERATION_LOGOUT: {
			Integer userKey = Integer.parseInt(msg.getNickname());
			if(nicks.containsValue(userKey)) {
				response = new DirMessage(LOGOUT_OK);
				for (Map.Entry<String, Integer> entry : nicks.entrySet()) {
					if(entry.getValue().equals(userKey)) {
		...
		case DirMessageOps.OPERATION_USERLIST: {
			try {
				response = new DirMessage(USERLIST_OK);
				response.setNickname("");
				for(String key : nicks.keySet()) {
					response.setNickname(response.getNickname() + "," + key);
				}
				response.setNickname(response.getNickname().substring(1));
				System.out.println("Userlist successful");
			} catch (Exception e) {
				response = new DirMessage(USERLIST_ERR);
				System.out.println("Userlist failed.");
			}
			return response;
