...
public class NFDirectoryServer {
	 ...
	 * registrados, etc.
	 */
	//Estructura para guardar usuarios que son servidores <UserKey,Port>
		private HashMap<Integer,Integer> registeredServers;
	...
	public NFDirectoryServer(double corruptionProbability) throws SocketException {
		...
		this.registeredServers = new HashMap<Integer,Integer>();
	...
	private DirMessage buildResponseFromRequest(DirMessage msg, InetSocketAddress clientAddr) {
		...
		case DirMessageOps.OPERATION_LIST: {
			...
			if(sessionKeys.containsKey(sessionKey)) {
				...
				for(String usr : nicks.keySet()) {
					String usrInfo = usr;
					if(registeredServers.containsKey(nicks.get(usr))) {
						usrInfo+=":Server";
						usrInfo+="\t Port: " + registeredServers.get(nicks.get(usr));
					}else {
						usrInfo+=":User";
					}
					username += usrInfo+",";
		...
		case DirMessageOps.OPERATION_REGISTER_SERVER: {
			...
			if(sessionKeys.containsKey(sessionKey)) {
				...
				registeredServers.put(sessionKey, port);
		...
		case DirMessageOps.OPERATION_UNREGISTER_SERVER: {
			int sessionKey = Integer.parseInt(msg.getSessionKey());
			if (registeredServers.containsKey(sessionKey)) {
				registeredServers.remove(sessionKey);
