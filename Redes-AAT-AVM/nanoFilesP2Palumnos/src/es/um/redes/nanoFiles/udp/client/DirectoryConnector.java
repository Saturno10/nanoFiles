...
public class DirectoryConnector {
	...
	public boolean logIntoDirectory(String nickname) throws IOException {
		...
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.LOGIN_OK)) {
			...
			System.err.println("ERROR: Login failed. That username is already being used.");
		}
	...
	public boolean logoutFromDirectory() throws IOException {
		...
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.LOGOUT_OK)) {
			...
			System.err.println("ERROR: Logout failed. You must be logged in to do logout and you cannot be a registered server.");
		}
		return success;
	}
	...
	public boolean registerServerPort(int serverPort) throws IOException {
		...
		assert (sessionKey != INVALID_SESSION_KEY);
		boolean success = false;
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_REGISTER_SERVER);
		messaje.setPort((new Integer(serverPort)).toString());
		messaje.setSessionKey(new Integer(sessionKey).toString());
		byte[] respuesta = sendAndReceiveDatagrams(messaje.toString().getBytes());
		String respuestaString = new String(respuesta);
		DirMessage respuestaMensaje = DirMessage.fromString(respuestaString);
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.REGISTER_SERVER_OK)) {
			System.out.println("Server registered on Directory");
			success = true;
		} else {
			System.err.println("ERROR: Server registration failed. You must be logged in to be registered");
		}

		return success;
	}

	public boolean unregisterServerPort() throws IOException {
		assert (sessionKey != INVALID_SESSION_KEY);
		boolean success = false;
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_UNREGISTER_SERVER);
		messaje.setSessionKey(new Integer(sessionKey).toString());
		byte[] respuesta = sendAndReceiveDatagrams(messaje.toString().getBytes());
		String respuestaString = new String(respuesta);
		DirMessage respuestaMensaje = DirMessage.fromString(respuestaString);
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.UNREGISTER_SERVER_OK)) {
			System.out.println("Server unregistered from Directory");
		} else {
	...
	public InetSocketAddress lookupServerAddrByUsername(String nick) throws IOException {
		...
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_DOWNLOADFROM);
		messaje.setNickname(nick);
		messaje.setSessionKey(Integer.toString(sessionKey));
		String request = messaje.toString();
		byte[] requestBytes = request.getBytes();
		byte[] respuesta = sendAndReceiveDatagrams(requestBytes);
		DirMessage respuestaMensaje = DirMessage.fromString(new String(respuesta));
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.DOWNLOADFROM_OK)) {
			String ip = respuestaMensaje.getIp();
	...
	public boolean publishLocalFiles(FileInfo[] files) throws IOException {
		assert (sessionKey != INVALID_SESSION_KEY);
		boolean success = false;
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_PUBLISH);
		messaje.setSessionKey(new Integer(sessionKey).toString());
		String filesStr = "";
		for (FileInfo file : files) {
			filesStr += file.fileHash + "," + file.fileName + "," + file.fileSize + ":";
		}
		messaje.setFiles(filesStr);
		byte[] respuesta = sendAndReceiveDatagrams(messaje.toString().getBytes());
		String respuestaString = new String(respuesta);
		DirMessage respuestaMensaje = DirMessage.fromString(respuestaString);
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.PUBLISH_OK)) {
			...
			System.err.println("ERROR: Files publication failed. You must be a registered server to publish files.");
		}
		return success;
	}
	...
	public FileInfo[] getFileList() throws IOException {
		...
		// TODO: Ver TODOs en logIntoDirectory y seguir esquema similar
		assert (sessionKey != INVALID_SESSION_KEY);
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_FILELIST);
		messaje.setSessionKey(new Integer(sessionKey).toString());
		byte[] respuesta = sendAndReceiveDatagrams(messaje.toString().getBytes());
		String respuestaString = new String(respuesta);
		DirMessage respuestaMensaje = DirMessage.fromString(respuestaString);
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.FILELIST_OK)) {
			...
			try{
				for (int i = 0; i < filelist.length; i++) {
					...
					filelist[i] = new FileInfo(filelistArray[i].split(",")[0], filelistArray[i].split(",")[1], Integer.parseInt(filelistArray[i].split(",")[2]), filelistArray[i].split(",")[3].replace('/', ','));
				}
	...
	public String[] getServerNicknamesSharingThisFile(String fileHash) throws IOException {
		...
		assert (sessionKey != INVALID_SESSION_KEY);
		DirMessage messaje = new DirMessage(DirMessageOps.OPERATION_SEARCH);
		messaje.setSessionKey(new Integer(sessionKey).toString());
		messaje.setHash(fileHash);
		byte[] respuesta = sendAndReceiveDatagrams(messaje.toString().getBytes());
		String respuestaString = new String(respuesta);
		DirMessage respuestaMensaje = DirMessage.fromString(respuestaString);
		if (respuestaMensaje.getOperation().equals(NFDirectoryServer.SEARCH_OK)) {
			...
			System.err.println("ERROR: Search failed: file not found.");
		}
